...
#     except Exception as e:
#         logger.error(f"Error in daily processing job: {str(e)}")
    
#     end_time = time.time()
#     duration = end_time - start_time
#     logger.info(f"=== Daily processing job completed in {duration:.2f} seconds ===")

# def start_scheduler():
#     """Start the scheduler to run the job daily"""
#     logger.info("Starting scheduler for daily application processing")
    
#     # Run the job every day at 2 AM
#     schedule.every().day.at("02:00").do(run_daily_process)
    
#     logger.info("Scheduler started. Will run daily at 02:00 AM")
